                                                ...
                                                let keyframe_points = curve["keyframe_points"]
                                                    ...
                                                    .filter_map(|kf| {
                                                        Some(scene_generics::KeyframePoint {
                                                            ...
                                                            handle_left: vec![kf["handle_left"][0].as_f64()? as f32, kf["handle_left"][1].as_f64()? as f32],
                                                            handle_left_type: kf["handle_left_type"].as_str()?.to_string(),
                                                            handle_right: vec![kf["handle_right"][0].as_f64()? as f32, kf["handle_right"][1].as_f64()? as f32],
                                                            handle_right_type: kf["handle_right_type"].as_str()?.to_string(),
                                                            interpolation: kf["interpolation"].as_str()?.to_string(),
                                                            co: vec![kf["co"][0].as_f64()? as f32, kf["co"][1].as_f64()? as f32],
...
impl SceneDiff {
    pub fn has_changes(&self) -> bool {
        !self.missing_objects.is_empty() || !self.new_objects.is_empty() || !self.missing_collections.is_empty() || !self.new_collections.is_empty()
    }
...
pub fn compare_scene_data(saved: &HashMap<String, Scene>, fresh: &HashMap<String, Scene>) -> SceneDiff {
    ...
    for (scene_name, saved_scene) in saved {
        if let Some(fresh_scene) = fresh.get(scene_name) {
            let saved_collections: std::collections::HashSet<_> = saved_scene.object_groups.iter().map(|g| &g.name).collect();
            let fresh_collections: std::collections::HashSet<_> = fresh_scene.object_groups.iter().map(|g| &g.name).collect();
            ...
            for saved_group in &saved_scene.object_groups {
                if let Some(fresh_group) = fresh_scene.object_groups.iter().find(|g| g.name == saved_group.name) {
                    let saved_objects: std::collections::HashSet<_> = saved_group.objects.iter().map(|o| &o.name).collect();
                    let fresh_objects: std::collections::HashSet<_> = fresh_group.objects.iter().map(|o| &o.name).collect();

                    for obj in saved_objects.difference(&fresh_objects) {
                        diff.missing_objects.push(format!("{}/{}/{}", scene_name, saved_group.name, obj));
                    }

                    for obj in fresh_objects.difference(&saved_objects) {
                        diff.new_objects.push(format!("{}/{}/{}", scene_name, fresh_group.name, obj));
                    }
                }
            }
        }
